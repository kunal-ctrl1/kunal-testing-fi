<script lang="ts">
    ...
    function collectIssues(): Issue[] {
        ...
        // Line-level checks (visual order = line order).
        lines.forEach((l, i) => {
            if (!l.item) return; // blank row — not an issue
            if (Number(l.qty) <= 0) {
                issues.push({
        ...
        // Charge-level: a DISCOUNT ledger picked but value is 0 (will be silently dropped by buildPayloadCharges).
        charges.forEach((c, i) => {
